...
int main(int argc, char **argv) {
    ...
    // 创建输出文件用于记录宽度信息
    std::ofstream width_out(output_path.string() + "/widths_summary.tsv");
    width_out << "file\tmin_width\tcols_at_min\tsolutions\n";
    ...
    for (int i = 0; i < ncell; i++) {
        ...
        if (temp.trans.size() > 10) {
            ...
            int min_width = placer.min_width + 2;
            int cols_at_min = placer.min_width;
            int solutions = 0;
            ...
            for (int width = placer.min_width; width <= placer.min_width + setting.relaxation; width++) {
                if (placer.solutions.find(width) != placer.solutions.end()) {
                    solutions += placer.solutions[width].size();
                }
            }
            
            out << "Min #CPP = " << min_width << std::endl;
            ...
            // 输出宽度信息到TSV文件
            std::string output_file_name = temp.name + "_w" + std::to_string(min_width) + ".txt";
            width_out << output_file_name << "\t" << min_width << "\t" << cols_at_min << "\t" << solutions << "\n";
            
            std::cout << "Cell: " << temp.name << ", Min Width: " << min_width << ", Solutions: " << solutions << std::endl;
        }
        else { 
            std::cout<<'\n'<<"case2"<<'\n'<<std::endl;
    ...
    std::cout << "\n宽度信息已保存到: " << output_path.string() << "/widths_summary.tsv" << std::endl;
    std::cout << "详细日志已保存到: " << output_path.string() << "/summary.txt" << std::endl;
